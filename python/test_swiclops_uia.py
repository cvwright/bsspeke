...
def register(homeserver, **kwargs):
...
def deactivate(homeserver, **kwargs):
    user_id = kwargs["user_id"]
    access_token = kwargs["access_token"]
...
if __name__ == "__main__":
    ...
    r1 = register(homeserver, **user_info)
    assert r1.status_code == 200

    r2 = login(homeserver, **user_info)
    assert r2.status_code == 200
    j2 = r2.json()
    access_token = j2.get("access_token", None)
    assert access_token != None
    user_info["access_token"] = access_token
    print("Logged in as [%s] with access token [%s]" % (user_id, access_token))

    r_whoami = whoami(homeserver, **user_info)
    assert r_whoami.status_code == 200
    j_whoami = r_whoami.json()
    whoami_userid = j_whoami["user_id"]
    print("Server says I am [%s]" % whoami_userid)
    assert whoami_userid == user_id

    r3 = deactivate(homeserver, **user_info)
